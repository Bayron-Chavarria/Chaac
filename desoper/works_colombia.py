...
class Work:
    ...
    def pagination(self):
        ...
        if j.status_code == 200:
            ...
            for l in set([d.get('level') for d in self.count_levels]):
                count = count.get(
                    [d.get('to_count_key')
                     for d in self.count_levels if d.get('level') == l][0]
                )
    ...
    def trabajos(self, email: str = " ", name: str = 'opnealexCO_publisher'):
        """
        Método para obtener todos los trabajos publicados por cada publisher.
        Se imprime los identificadores de cada publisher a medida que se hace el requests.
